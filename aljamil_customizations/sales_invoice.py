...
def send_discount_approval_notifications(doc, method=None):
    ...
    This function is called before_save on Sales Invoice.
    """
        ...
        if has_branch_permission:
            ...
            if not existing_log:
                try:
                    ...
                    frappe.log_error(
                        "[sales_invoice.py] method: send_discount_approval_notifications",
                        "Discount Approval Notification",
                    )
                except Exception as e:
                    frappe.log_error(
                        f"[sales_invoice.py] method: send_discount_approval_notifications - Error: {str(e)}",
