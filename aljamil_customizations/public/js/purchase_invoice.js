	...
	refresh(frm) {
		...
		if (frm.doc && frm.doc.docstatus === 1) {
			...
			if (frm._multi_add_button_pi && frm._multi_add_button_pi.remove) {
				...
				frm._multi_add_button_added_pi = false;
			}
			return;
		}

		if (!frm._multi_add_button_added_pi) {
			frm._multi_add_button_pi = grid.add_custom_button(
				__('📦 Multi Add Items'),
				function () { open_multi_add_dialog_for_native_table_pi(frm); },
				'bottom'
			);
			frm._multi_add_button_added_pi = true;
		}
	}
});

function open_multi_add_dialog_for_native_table_pi(frm) {
	console.log('[purchase_Invoice.js] open_multi_add_dialog_for_native_table_pi');
	...
	search_field.df.onchange = () => {
		...
		if (!txt || !wh) return;
		page_state.page = 0;
		search_items_with_stock_for_native_pi(txt, wh, $tbody, d, page_state.page);
	};

	d.show();
}

function search_items_with_stock_for_native_pi(txt, warehouse, $tbody, d, page = 0) {
	$tbody.empty();
	...
	frappe.call({
		...
		callback(r) {
			...
			if (!results_mapped.length) {
				...
				render_multi_pager_pi(d, page, 0);
				return;
			}
			...
			frappe.call({
				...
				callback(b) {
					...
					render_multi_pager_pi(d, page, results.length);
				}
			});
		}
	});
...
function render_multi_pager_pi(d, page, results_length) {
	...
	if (page > 0) {
		$prev.on('click', () => {
			...
			const wh = d.get_value('warehouse');
			const $tbody = d.get_field('results').$wrapper.find('tbody');
			search_items_with_stock_for_native_pi(txt, wh, $tbody, d, page - 1);
		});
		$pager.append($prev);
	}
